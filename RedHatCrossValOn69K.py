...
train_group_df_train = train_group_df.sample(frac=0.80,random_state=127937)
...
X_train = train[train.group_1.isin(train_group_df_train.group_1.values)]
X_eval = train[~train.group_1.isin(train_group_df_train.group_1.values)]

Y_train = X_train.outcome
Y_eval = X_eval.outcome

print len(Y_train),len(Y_eval)

X_train = X_train[features].drop(['people_id','activity_id'], axis = 1)
X_eval = X_eval[features].drop(['people_id','activity_id'], axis = 1)
gc.collect()
...
X_test1['grp_actf'] = X_test1.groupby('group_1')['group_1'].transform('count')
...
del X_train['day_y']
del X_train['month_y']
del X_train['quarter_y']
del X_train['isweekend_y']
del X_eval['year_y']
del X_eval['day_y']
del X_eval['month_y']
del X_eval['quarter_y']
del X_eval['isweekend_y']
del X_test1['year_y']
del X_test1['day_y']
del X_test1['month_y']
...
dtrain_eval = xgb.DMatrix(X_eval,label=Y_eval)
gc.collect()
...
submit.to_csv('SubmissionCV.csv', index=False)
